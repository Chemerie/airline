...
class FlightTestCase(TestCase):
	def setUp(self):
		#Create airports
		a1 = Airports.objects.create(code="AAA", city="City A")
		a2 = Airports.objects.create(code="BBB", city="City B")
	...
	def text_valid_flight_page(self):
		a1 = Airports.objects.get(code="AAA")
		f = Flight.objects.get(origin=a1, destination=a1)
	...
	def test_flight_page_passengers(self):
		...
		response = c.get(f"/flights/{f.id}")
		self.assertEqual(response.status_code, 200)
		self.assertEqual(response.context["passenegers"].count(), 1)
